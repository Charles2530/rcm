...
The Wan2.1 VAE and umT5 text encoder can be obtained from the official [Wan-Video/Wan2.1](https://github.com/Wan-Video/Wan2.1) repository. To download all required checkpoints into the `model/Wan2.1-T2V-distill/` directory, run:

```bash
mkdir model/Wan2.1-T2V-distill
cd model/Wan2.1-T2V-distill
...
PYTHONPATH=.  python rcm/inference/wan2pt1_t2v_rcm_infer.py \
    --dit_path model/Wan2.1-T2V-distill/rCM_Wan2.1_T2V_14B_480p.pt \
    --model_size 14B \
    --num_samples 5 \
...
```bash
PYTHONPATH=.  python rcm/inference/wan2pt1_t2v_rcm_infer.py \
    --dit_path model/Wan2.1-T2V-distill/rCM_Wan2.1_T2V_14B_720p.pt \
    --model_size 14B \
    --num_samples 4 \
    --resolution 720p \
    --sigma_max 120 \
...
PYTHONPATH=.  python rcm/inference/wan2pt2_i2v_rcm_infer.py \
    --high_noise_model_path model/Wan2.1-T2V-distill/Wan2.2-I2V-A14B-high-rCM4.0-merged.pt \
    --low_noise_model_path model/Wan2.1-T2V-distill/Wan2.2-I2V-A14B-low-rCM1.0-merged.pt \
    --num_samples 2 \
